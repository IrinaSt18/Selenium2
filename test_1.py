...
def test_step_3(browser, title, content, description):
    ...
    testpage.create_post_title(title)
    testpage.create_post_description(description)
    testpage.create_post_content(content)
    time.sleep(test_data['sleep_time'])
    testpage.click_create_post_button()
    time.sleep(test_data['sleep_time'])
    testpage.go_to_site()
    assert testpage.get_created_post() == title
